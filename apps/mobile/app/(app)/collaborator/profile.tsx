...
import { Mail, Briefcase, LogOut, Building2, Pencil, Camera, Star, Lock } from "lucide-react-native";
...
export default function CollaboratorProfileScreen(): ReactElement {
  ...
  const pickAvatar = async () => {
    ...
    if (!perm.granted) {
      Alert.alert("Permissão", "Ativa o acesso à galeria nas definições.");
      return;
    }
  ...
  const avatarUri = resolveMediaUrl(display?.avatar_url ?? user?.avatarUrl ?? null);
    ...
    <ScrollView style={{ flex: 1, backgroundColor: "#F5F7FA" }} contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ backgroundColor: NAVY, paddingTop: 24, paddingBottom: 56, alignItems: "center", paddingHorizontal: 24 }}>
        ...
        <View style={{ backgroundColor: "rgba(42,191,191,0.2)", borderRadius: 20, paddingHorizontal: 14, paddingVertical: 4, marginTop: 6 }}>
          <Text style={{ color: TEAL, fontSize: 13, fontWeight: "700" }}>{roleLabel[user?.role ?? ""] ?? user?.role ?? ""}</Text>
        </View>
      </View>
      ...
      <View style={{ backgroundColor: "#fff", marginHorizontal: 16, marginTop: -28, borderRadius: 20, paddingHorizontal: 16, paddingTop: 4, shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 6 }}>
        ...
        <InfoRow icon={Briefcase} label="Especialidade" value={display?.specialty ?? user?.specialty ?? "—"} />
        <InfoRow icon={Building2} label="Departamento" value={display?.department_name ?? "—"} />
        {display?.contract_hours_week != null && (
          <InfoRow icon={Star} label="Horas contratuais / semana" value={`${display.contract_hours_week}h`} />
        )}
      </View>
      ...
      <View style={{ marginHorizontal: 16, marginTop: 16, gap: 10 }}>
        <TouchableOpacity
          onPress={() => router.push("/(app)/collaborator/edit-profile")}
        ...
        >
          ...
          <Text style={{ fontSize: 15, fontWeight: "600", color: "#1E293B", flex: 1 }}>Editar perfil</Text>
        </TouchableOpacity>

        <TouchableOpacity
          onPress={() => router.push("/(app)/collaborator/change-password")}
          style={{ flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: "#fff", borderRadius: 16, padding: 16, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 3 }}
          activeOpacity={0.8}
        >
          ...
          <Text style={{ fontSize: 15, fontWeight: "600", color: "#1E293B", flex: 1 }}>Alterar password</Text>
        </TouchableOpacity>
